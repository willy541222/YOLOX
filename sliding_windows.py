...
image_path = os.listdir(args["image"])
...
for image in image_path:
    ...
    image = cv2.imread("./assets/img/{}".format(image))
    if image.shape[0] == 1080:
        # 1920*1080 (W*H) y : 440 x : 640
        i = 0
        print("Croping the image : {}".format(image_name))
        for (x, y, window) in sliding_window(image, ystepsize=440, xstepsize=640, windowsize=(winW, winH)):
            ...
            cv2.imwrite("./assets/crop/{}_{}.jpg".format(image_name, i), window)
            i += 1
            cv2.waitKey(1)
            time.sleep(1)
